  ...
  describe("initialize", () => {
    it("should call useMultiFileAuthState with correct path", async () => {
      ...
      expect(mockUseMultiFileAuthState).toHaveBeenCalledWith(
        join(testSessionPath, testInstanceId)
      );
    });
    ...
    it("should handle multiple instances with different paths", async () => {
      ...
      expect(mockUseMultiFileAuthState).toHaveBeenNthCalledWith(
        1,
        join(testSessionPath, "instance-1")
      );
      expect(mockUseMultiFileAuthState).toHaveBeenNthCalledWith(
        2,
        join(testSessionPath, "instance-2")
      );
    });
